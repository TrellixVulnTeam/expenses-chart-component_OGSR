...
fetch("../dist/data.json").then(function (resp) {
  ...
  var _loop = function _loop(i) {
    ...
    var width = Math.ceil(chartWrapper.offsetWidth / data.length);
    var height = Math.ceil(data[i].amount);
    ...
    barChart.addEventListener("mouseover", function () {
      amount.classList.add("span-amount");
      amount.textContent = "$" + data[i].amount;
    });
    barChart.addEventListener("mouseout", function () {
  ...
  var today = date.toLocaleString('en-GB', options).toLowerCase();
  var todaySpanNumber = barChartDayArray.indexOf(today);
  var greenBar = chartWrapper.children[todaySpanNumber];
